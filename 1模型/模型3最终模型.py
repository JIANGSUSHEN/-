...
def save_results(theta, cost_history, output_dir, header):
    """保存模型结果和损失曲线"""
    ...
    for i in range(1, len(theta)):
        equation += f" + ({theta[i]:.4f})*{header[i - 1]}"
    print(equation)

    # 保存数学模型到文件
    model_equation_file = os.path.join(output_dir, "model_equation.txt")
    with open(model_equation_file, "w") as f:
        f.write("Mathematical Model:\n")
        f.write(equation + "\n")
    print(f"Model equation saved to: {model_equation_file}")


if __name__ == "__main__":
    # 文件路径由用户定义
    csv_file_path = r"D:\BaiduNetdiskDownload\Harvard university 人工智能\多元线性下降模型（销量预测）\1模型\advertising (1).csv"
    output_dir = r"D:\BaiduNetdiskDownload\Harvard university 人工智能\多元线性下降模型（销量预测）\模型训练结果\模型3"
